...
while iter < 6:
    ...
    for f_i in f_count.keys():
        for e_j in e_count.keys():
            ...
            # Add-N smoothing operation here.  The null_constant is set to 1 because we are using a test set as evaluation.
            # Otherwise, this serves as a hyperparameter.
...
for (f, e) in bitext:
    ...
    # Would be used normally if not against a test set
    # for i in range(null_constant):
    #     f.insert(i, "NULL")
    #     e.insert(i, "NULL")
    ...
    for (i, f_i) in enumerate(f):
        ...
        if subscript != -1:
            ...
            # This would be (i-null_constant, subscript-null_constant))
            # if used normally if not against a test set
    sys.stdout.write("\n")
